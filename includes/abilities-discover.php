...
function register_discover_abilities(): void {
	Helpers::register(
		'amelia/get-status',
		array(
			'label'       => __( 'Get Status', 'harudigi-booking-abilities-for-amelia' ),
			'description' => __( 'Amelia version, counts, native vs helper abilities.', 'harudigi-booking-abilities-for-amelia' ),
	...
	Helpers::register(
		...
		array(
			'label'       => __( 'List API Surface', 'harudigi-booking-abilities-for-amelia' ),
			'description' => __( 'Native Amelia MCP vs helper gap fillers.', 'harudigi-booking-abilities-for-amelia' ),
	...
	Helpers::register(
		'amelia/get-stats',
		array(
			'label'        => __( 'Get Stats', 'harudigi-booking-abilities-for-amelia' ),
			'description'  => __( 'Dashboard stats. Optional date_from/date_to.', 'harudigi-booking-abilities-for-amelia' ),
	...
	Helpers::register(
		'amelia/get-entities',
		array(
			'label'        => __( 'Get Entities', 'harudigi-booking-abilities-for-amelia' ),
			'description'  => __( 'Bulk admin entities dump (large, may include PII). Prefer targeted get-* abilities. Requires confirm=true.', 'harudigi-booking-abilities-for-amelia' ),
	...
	Helpers::register(
		...
		array(
			'label'       => __( 'Get Settings Summary', 'harudigi-booking-abilities-for-amelia' ),
			'description' => __( 'Non-secret settings. Secrets redacted.', 'harudigi-booking-abilities-for-amelia' ),
...
function ability_get_entities( array $input = array() ) {
	...
	if ( is_wp_error( $ok ) ) {
		return new \WP_Error(
			'confirm_required',
			__( 'Bulk get-entities refused. Set confirm=true only when a full dump is required. Prefer get-service / get-customer / list-* with pagination.', 'harudigi-booking-abilities-for-amelia' )
		);
	}
