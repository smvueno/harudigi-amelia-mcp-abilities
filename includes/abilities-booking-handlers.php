...
function ability_update_payment( array $i = array() ) {
	...
	if ( ! $patch ) {
		return new \WP_Error( 'invalid_fields', __( 'Provide gateway, status, amount, and/or transactionId to update.', 'harudigi-amelia-mcp-abilities' ) );
	}
...
function ability_update_appointment( array $i = array() ) {
	...
	if ( ! is_array( $existing ) ) {
		return new \WP_Error( 'amelia_not_found', __( 'Appointment not found.', 'harudigi-amelia-mcp-abilities' ) );
	}
...
function ability_update_booking_status( array $i = array() ) {
	...
	if ( ! in_array( $type, array( 'appointment', 'event' ), true ) ) {
		return new \WP_Error( 'invalid_type', __( 'type must be appointment or event.', 'harudigi-amelia-mcp-abilities' ) );
	}
